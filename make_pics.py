...
def make_eeg1_spec(startInd, lsplit, Mlength, M, savepath, eeg1):
	...
	while ind1 < limit:
		eeg_start = 2500*ind1
		eeg_end = 2500*(ind1+1)
		subeeg = eeg1[eeg_start:eeg_end]
		xes = np.arange(0,2500)
		fig1 = plt.figure(figsize=(2.56,2.56))
...
def make_eeg2_spec(startInd, lsplit, Mlength, M, savepath, eeg2):
	...
	while ind1 < limit:
		eeg_start = 2500*ind1
		eeg_end = 2500*(ind1+1)
		subeeg = eeg2[eeg_start:eeg_end]
		xes = np.arange(0,2500)
		fig2 = plt.figure(figsize=(2.56,2.56))
		plt.plot(xes, subeeg, lw=1, color='k')
		plt.axis('off')
...
def make_emg_spec(startInd, lsplit, Mlength, M, savepath, emg):
	...
	while ind1 < limit:
		eeg_start = 2500*ind1
		eeg_end = 2500*(ind1+1)
		subeeg = emg[eeg_start:eeg_end]
		xes = np.arange(0,2500)
		fig3 = plt.figure(figsize=(2.56,2.56))
		plt.plot(xes, subeeg, lw=1, color='k')
		plt.axis('off')
